...
Point Cloud based Fringe Pattern Generation with Example
*/
...
#include "../ophgen_pointcloud/ophgen_pointcloud.h"
...
int main(int argc, char **argv)
{
	...
	int n_x = config.nx;
	int n_y = config.ny;
	int n_points = openholo::ophLoadPointCloud(INPUT_MODEL, model);
	if (n_points == -1) {
	...
	double time = openholo::ophGenCghPointCloud(model, n_points, config, fringe);
