...
GLuint ilOgl::BindTex(ilImage &Image)
{
	...
	glPixelStorei(GL_UNPACK_ALIGNMENT, p);
	return ret;
}
...
{
	Image.Bind();
	return false; //ilutLoadResource(hInst, ID, ResourceType, IL_TYPE_UNKNOWN);
}

ILboolean ilWin32::GetResource(ilImage &Image, HINSTANCE hInst, ILint ID, char *ResourceType, ILenum Type)
{
	Image.Bind();
	return false; //ilutLoadResource(hInst, ID, ResourceType, Type);
}
