	...
	for (p = 0; p < depth_sz; ++p)
	{
		...
		{			
			...
			if (Propagation_Method_ == 0)
				Propagation_AngularSpectrum('S', u_o, -temp_depth);
...
void HologramGenerator::fftwShift(Complex* src, Complex* dst, int nx, int ny, int type, bool bNomalized, char domain)
{
